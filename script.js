...
if (homeHeroTitle) {
  db.collection("site").doc("home").get().then(doc => {
    ...
    if (homeHero && settings.heroBackground) {
      homeHero.style.backgroundImage = `linear-gradient(to right, rgba(7,6,5,.2), rgba(7,6,5,.82)), linear-gradient(to bottom, rgba(7,6,5,.05), #070605), url("${settings.heroBackground}")`;
    }
...
async function loadGlobalSearchIndex() {
  if (!globalSearchInput || !globalSearchResults || !window.db) return;
